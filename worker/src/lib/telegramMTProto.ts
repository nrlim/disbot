        ...
        const syncPromises = Array.from(configsByToken.entries()).map(async ([token, sessionConfigs]) => {
            ...
            if (session) {
                ...
                if (!session.client.connected) {
                    logger.warn({ token: token.substring(0, 10) + '...' }, 'Telegram client disconnected — destroying session to force clean re-creation');
                    await this.destroySession(token, session);
                    return; // Skip further processing this cycle, will re-create next sync
                }
            } else {
                                    ...
                                    if (uniqueChatIds.length > 0) {
                                        ...
                                        for (const chatId of uniqueChatIds) {
                                            try {
                                                ...
                                                if (entity) {
                                                    // ─── linkedChatId Discovery (Channel → Group) ───
                                                    // Telegram Channels (broadcasts) and their linked Discussion
                                                    // Groups are SEPARATE entities with DIFFERENT IDs.
                                                    // UpdateNewChannelMessage fires for admin broadcasts (the Channel),
                                                    // but member chats happen in the linked Group (different ID).
                                                    //
                                                    // If a configured source is a Channel with a linkedChatId,
                                                    // log the Group ID so the operator can add it to the Dashboard.
                                                    const linkedChatId = (entity as any).linkedChatId?.toString();
                                                    if (linkedChatId) {
                                                        logger.info({
                                                            chatId,
                                                            entityTitle: (entity as any).title || '(unknown)',
                                                            entityClass: (entity as any).className,
                                                            linkedGroupId: `-100${linkedChatId}`,
                                                            linkedGroupIdBare: linkedChatId
                                                        }, '[Telegram] 🔗 Discovery: Source is a Broadcast Channel. Its Linked Discussion Group ID is: -100' + linkedChatId + '. Use this Group ID in Dashboard to mirror member chats.');
                                                    }
            ...
            if (session) {
                ...
                if (!session.keepAliveInterval) {
                    session.keepAliveInterval = setInterval(async () => {
                        // In Headless GramJS, dealing with dropped sessions via .connect() often hangs or enters buggy states.
                        // The safest logic is to completely destroy the TelegramClient object if it drops or hangs, 
                        // and let the next sync cycle (every 30s) create a 100% fresh connection.

                        try {
                            if (!session.client.connected) {
                                logger.warn({ token: token.substring(0, 8) + '...' }, 'Active Keep-Alive: Client disconnected. Destroying to allow clean re-create.');
                                await this.destroySession(token, session);
                                return;
                            } else {
                            ...
                            }, 'Active Keep-Alive: Ping failed');

                            // If ping fails 2 times, destroy it.
                            if (session.reconnectFailures >= 2) {
                                logger.warn({ token: token.substring(0, 8) + '...' },
                                    `Active Keep-Alive: 2 consecutive ping failures — destroying session for clean re-creation on next sync`);
                                await this.destroySession(token, session);
                                return;
                            }
                        }
